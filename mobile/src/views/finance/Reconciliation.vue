...
<script setup>
  ...
  const pageConfig = computed(() => ({
    ...
    fields: {
      id: 'id',
      title: (item) => item.relatedParty || item.description || '待对账交易',
      subtitle: (item) => `${item.transactionNumber || ''} · ${item.accountName || ''}`,
      icon: 'clipboard-check',
      details: [
        { label: '交易金额', field: 'amount', format: 'money', prefix: '¥' },
        { label: '交易日期', field: 'transactionDate', format: 'date' },
        { label: '交易类型', field: 'transactionType' }
      ]
    }
  }))
