...
/**
 * CreateOutbound.vue
 * @description 新建出库单页面 - 支持物料搜索选择和仓库动态加载
 * @date 2026-04-14
 * @version 2.0.0
 */
-->
<template>
  <div class="create-page">
    <NavBar title="新建出库单" left-arrow @click-left="$router.go(-1)" />
...
<script setup>
  ...
  const onTypeConfirm = ({ selectedOptions }) => {
    form.outbound_type = selectedOptions[0].text
    form.outbound_type_value = selectedOptions[0].value
  ...
  const onSubmit = async () => {
    ...
    try {
      const payload = {
        outbound_type: form.outbound_type_value,
