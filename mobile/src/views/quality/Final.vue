...
/**
 * Final.vue
 * @description 成品检验列表页面 — 统一 Unified 暗色风格
 * @date 2025-12-27
 * @version 2.0.0
 */
-->
<template>
  <div class="inspection-page">
    <NavBar title="成品检验" left-arrow @click-left="$router.go(-1)">
      <template #right>
            ...
            <div v-for="inspection in inspections" :key="inspection.id" class="inspection-item">
              <div class="inspection-header" @click="viewInspection(inspection)">
                <div class="inspection-info">
                  ...
                  <div class="batch-number">批次: {{ inspection.batch_no || '--' }}</div>
                </div>
              ...
              <div class="inspection-content">
                <div class="material-info">
                  ...
                  <div class="supplier-name">规格: {{ inspection.item_specs || '--' }}</div>
                </div>
...
<script setup>
  ...
  const loadInspections = async (isRefresh = false) => {
    ...
    try {
      ...
      const response = await qualityApi.getFinalInspections(params)
      ...
      console.error('加载成品检验列表失败:', error)
      finished.value = true
    } finally {
      loading.value = false
  ...
  const viewInspection = (inspection) => {
    router.push({
      path: `/quality/final/${inspection.id}`,
