...
function exercise(scenario) {
  ...
  try {
    ...
    const successful = scenario === 'success' || scenario === 'retry';
    ...
    for (const service of ['backend', 'frontend', 'mobile']) {
      assert.equal(finalState.containers[service], successful ? 'release-' + release : oldTag, result.stderr);
    }
    assert.match(fs.readFileSync(path.join(target, '.env'), 'utf8'), /KEEP=test/);
    if (['migration', 'container', 'health', 'version'].includes(scenario)) {
...
for (const scenario of ['success', 'retry', 'build', 'migration', 'container', 'health', 'version', 'stale', 'tampered', 'locked']) {
