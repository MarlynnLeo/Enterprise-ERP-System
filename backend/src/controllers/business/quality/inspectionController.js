...
const inspectionController = {
    ...
    async updateInspection(req, res) {
        ...
        try {
            ...
            const [[source]] = await connection.query('SELECT reference_id, source_type, inspection_type FROM quality_inspections WHERE id = ? AND deleted_at IS NULL', [id]);
            if (source?.inspection_type === 'incoming' && source.source_type !== 'outsourced_receipt') {
                await connection.query('SELECT id FROM purchase_orders WHERE id = ? FOR UPDATE', [source.reference_id]);
            }
