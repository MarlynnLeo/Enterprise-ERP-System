...
const purchaseReceiptItemMap = {
  fromApi(body = {}) {
    ...
    const row = {
      ...
      price: body.price == null && body.unitPrice == null ? undefined : line.price,
      amount,
      tax_rate: body.taxRate,
  ...
  toApi(row) {
    ...
    return {
      ...
      amount: toNumber(row.amount_excluding_tax ?? row.amount, 0),
      taxAmount: row.tax_amount != null ? toNumber(row.tax_amount, 0) : null,
      totalAmount: row.total_amount != null ? toNumber(row.total_amount, 0) : null,
      taxRate: row.tax_rate != null ? toNumber(row.tax_rate, 0) : null,
      remarks: row.remarks ?? null,
...
const purchaseReceiptMap = {
  ...
  toApi(row) {
    ...
    const api = {
      ...
      fromInspection: Boolean(row.from_inspection),
...
const purchaseReturnItemMap = {
  ...
  toApi(row) {
    ...
    return {
      ...
      quantity: toNumber(row.quantity, 0),
      receivedQuantity: toNumber(row.quantity, 0),
...
const purchaseReturnMap = {
  ...
  toApi(row) {
    ...
    const api = {
      ...
      reason: row.reason ?? row.return_reason ?? null,
      returnReason: row.reason ?? row.return_reason ?? null,
      supplierId: row.supplier_id ?? null,
      supplierName: row.supplier_name ?? null,
      warehouseId: row.warehouse_id ?? null,
