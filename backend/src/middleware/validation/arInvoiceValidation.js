/**
 * arInvoiceValidation.js
 * @description 应收发票相关路由的 express-validator 验证中间件
 * @date 2026-06-15
 * @version 1.0.0
 */
 ...
 * 创建应收发票验证
 */
const createARInvoiceValidation = [
  body('customer_id')
    .isInt({ min: 1 })
 ...
 * 更新应收发票验证
 */
const updateARInvoiceValidation = [
  param('id').isInt({ min: 1 }).withMessage('发票ID必须是正整数'),
  ...createARInvoiceValidation,
];
...
const createReceiptValidation = [
  ...
  body('amount')
    .isFloat({ min: 0.01 })
    .withMessage('收款金额必须大于0'),

  body('receiptDate')
    .notEmpty()
    .withMessage('收款日期不能为空')
    .matches(/^\d{4}-\d{2}-\d{2}$/)
    .withMessage('收款日期格式必须为 YYYY-MM-DD'),
...
module.exports = {
  createARInvoiceValidation,
  updateARInvoiceValidation,
  createReceiptValidation,
  getARInvoicesValidation,
};
