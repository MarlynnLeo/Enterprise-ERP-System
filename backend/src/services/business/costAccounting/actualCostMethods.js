  ...
  async calculateActualCost(productionOrderId, externalConn = null) {
    ...
    try {
      ...
      if (finishedGoodsUnitCost > 0 && order.product_id) {
        ...
        const [taskLedgerRows] = await connection.execute(
        ...
        for (const ledger of taskLedgerRows) {
          await InventoryValuationAdjustmentService.record(connection, {
            ledgerId: ledger.id,
            postingDocumentId: ledger.posting_document_id,
            adjustmentType: 'production_actual_cost',
            newUnitCost: finishedGoodsUnitCost,
            ledgerSnapshot: ledger,
            reason: `生产任务 ${order.code || productionOrderId} 实际成本重估`,
            actorId: await resolveActorUserId(connection),
            actorLabel: order.manager || 'system',
          });
        }
