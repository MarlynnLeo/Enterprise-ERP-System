...
module.exports = {
  async assertCoreClosingControls(connection, period) {
    ...
    const [[failedJobs]] = await connection.execute(
      `SELECT COUNT(*) AS count
          ...
          WHERE status IN ('pending','retrying','failed','ignored')
            AND (
              LOWER(task_name) REGEXP '^(finance:|financeintegration:|eventbus:inventory_posting_approved$)'
              OR LOWER(task_name) REGEXP 'cost|voucher|tax|purchase|sales|production|财务|成本|凭证|税|采购|销售|生产'
            )`
    );
    ...
    const [[inventoryPostings]] = await connection.execute(
      `SELECT COUNT(*) AS count
           FROM inventory_posting_documents d
          WHERE COALESCE(d.is_legacy, 0) = 0
            AND d.transaction_date BETWEEN ? AND ?
            AND (
              d.finance_status IN ('pending', 'rejected')
              OR (d.finance_status = 'approved' AND EXISTS (
                SELECT 1 FROM inventory_posting_lines l
                 WHERE l.posting_document_id = d.id
                   AND l.posted_quantity IS NULL
              ))
            )`,
      [period.start_date, period.end_date]
    );
    if (Number(inventoryPostings.count || 0) > 0) {
      throw new Error(
        `本期有 ${inventoryPostings.count} 张库存过账单尚未完成财务审核或正式入账，不能关账`
      );
    }
    ...
    try {
      ...
      const normalizedEntries = entries.map((entry) => {
        ...
        return {
          ...
          posting_ready: entryDateValid && postingDateValid && Boolean(diagnostic.posting_ready),
  ...
  async updateUnpostedEntryDates(entryId, payload) {
    ...
    try {
      ...
      const postingDate = this.normalizeDateInput(
        payload.posting_date || payload.entry_date,
        '过账日期'
      );

      if (
        !this.isDateWithinPeriod(entryDate, entry) ||
        !this.isDateWithinPeriod(postingDate, entry)
      ) {
        throw new Error(
