...
describe('operationColumnAutoWidth', () => {
  ...
  afterEach(() => {
    operationColumnAutoWidth.destroyOperationColumnAutoWidth()
  ...
  test('starts the shared live measurement service for existing tables', async () => {
    ...
    Node.prototype.appendChild = function patchedAppendChild(node) {
      if (node?.getAttribute?.('data-erp-operation-measure-probe') === 'true') appendCount += 1
    ...
    operationColumnAutoWidth.startOperationColumnAutoWidth(document.body)
  ...
  test('remeasures when visible row actions change', async () => {
    document.body.innerHTML = tableMarkup
    vi.stubGlobal('requestAnimationFrame', (callback) => setTimeout(callback, 0))
    vi.stubGlobal('cancelAnimationFrame', (id) => clearTimeout(id))
    vi.stubGlobal('ResizeObserver', class {
    ...
    Object.defineProperty(HTMLElement.prototype, 'offsetWidth', {
      configurable: true,
      get() {
        if (this.getAttribute?.('data-erp-operation-measure-probe') === 'true') return this.children.length * 50
        return 50
      }
    })
    Object.defineProperty(HTMLElement.prototype, 'offsetHeight', { configurable: true, get: () => 24 })
