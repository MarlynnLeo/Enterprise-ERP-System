// 重新导出 axios 实例
export { api, fastApi } from './axiosInstance';
export { default } from './axiosInstance';

// 导出所有模块
export * from '../api/sales';
export * from '../api/material';
export * from '../api/bom';
export * from '../api/supplier';
export * from '../api/location';
export * from '../api/baseData';
export * from '../api/inventory';
export * from '../api/production';
export * from '../api/purchase';
export * from '../api/quality';
export * from '../api/afterSales';
export * from '../api/system';
export * from '../api/user';
export * from '../api/finance';
export * from '../api/equipment';
export * from '../api/hr';
export * from '../api/nonconformingProductApi';
export { default as nonconformingProductApi } from '../api/nonconformingProductApi';
export * from '../api/modules/business/equipmentMonitoring';
export * from '../api/common';
export * from '../api/workflow';
export * from '../api/contract';
export * from '../api/enhanced';
