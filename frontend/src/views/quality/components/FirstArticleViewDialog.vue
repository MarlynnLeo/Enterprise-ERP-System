...
<template>
  <AppDialog v-model="dialogVisible" title="首检详情" mode="view">
    <el-descriptions :column="2" border>
      ...
      <el-descriptions-item label="产品编码">{{ inspection?.productCode }}</el-descriptions-item>
      ...
      <el-descriptions-item label="合格数量">{{ inspection?.qualifiedQuantity || 0 }} {{ inspection?.unit }}</el-descriptions-item>
      <el-descriptions-item label="不合格数量">{{ inspection?.unqualifiedQuantity || 0 }} {{ inspection?.unit }}</el-descriptions-item>
      <el-descriptions-item label="首检结果">
        <el-tag :type="getResultType(inspection?.firstArticleResult)">
          {{ getResultText(inspection?.firstArticleResult) }}
        </el-tag>
      </el-descriptions-item>
      <el-descriptions-item label="可继续生产">
        <el-tag :type="inspection?.productionCanContinue ? 'success' : 'info'" size="small">
          {{ inspection?.productionCanContinue ? '是' : '否' }}
        </el-tag>
      </el-descriptions-item>
      <el-descriptions-item label="检验员">{{ inspection?.inspectorName || '-' }}</el-descriptions-item>
      <el-descriptions-item label="计划日期">{{ formatDate(inspection?.plannedDate) }}</el-descriptions-item>
      <el-descriptions-item label="实际日期">{{ formatDate(inspection?.actualDate) }}</el-descriptions-item>
