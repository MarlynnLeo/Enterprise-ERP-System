...
/**
 * AutomationTab.vue
 * @description 财务自动化任务管理 Tab — 从 FinanceAutomation.vue 提取
 */
-->
<template>
...
// 表单数据
const depreciationForm = reactive({ month: '' });
...
const getFailedJobStatusName = (status) => {
  const statusMap = { pending: '待处理', retrying: '重试中', resolved: '已处理', ignored: '已忽略' };
  return statusMap[status] || status || '-';
};

const getFailedJobStatusType = (status) => {
  const typeMap = { pending: 'danger', retrying: 'warning', resolved: 'success', ignored: 'info' };
...
const formatFailedJobPayload = (payload) => {
  ...
  const keys = ['receiptNo', 'orderNo', 'outboundNo', 'returnNo', 'receiptId', 'salesOrderId', 'outboundId', 'returnId'];
  const summary = keys
...
const loadFailedJobs = async () => {
  ...
  try {
    const response = await financeApi.automation.getFailedJobs({
      status: failedJobStatus.value, page: 1, pageSize: 20,
    });
...
const retryFailedJobs = async () => {
  ...
  try {
    ...
    ElMessage.success(`已执行重试：处理 ${result.retried || 0} 条，成功 ${result.resolved || 0} 条`);
...
const loadProductionTasks = async () => {
  try {
    const response = await productionApi.getProductionTasks({
      status: 'completed', pageSize: 50, page: 1,
    });
...
const getTaskTypeName = (type) => {
  const typeMap = {
    depreciation: '折旧计提', periodEnd: '期末结转', production: '生产成本',
    repair: '异常补生成', financeYearEnd: '财务年度结转',
    inventoryYearEnd: '仓库年度结存', inventoryYearFreeze: '仓库结存冻结',
  };
...
const getTaskTypeColor = (type) => {
  const colorMap = {
    depreciation: 'primary', periodEnd: 'success', production: 'warning',
    repair: 'primary', financeYearEnd: 'danger',
    inventoryYearEnd: 'info', inventoryYearFreeze: 'warning',
  };
...
const loadYearEndStatus = async () => {
  if (!yearEndForm.year) { financeYearStatus.value = {}; return; }
  try {
    const response = await financeApi.automation.getFinanceYearEndStatus(yearEndForm.year);
    if (response?.data) { financeYearStatus.value = response.data; }
  } catch (error) {
...
const executeFinanceYearEnd = async () => {
  if (!yearEndForm.year) { ElMessage.warning('请选择会计年度'); return; }
  try {
...
const loadInventoryYearEndStatus = async () => {
  if (!inventoryYearEndForm.year) { inventoryYearStatus.value = {}; return; }
  try {
    const response = await inventoryApi.getYearEndStatus(inventoryYearEndForm.year);
    if (response?.data) { inventoryYearStatus.value = response.data; }
  } catch (error) {
...
const executeInventoryYearEnd = async () => {
  if (!inventoryYearEndForm.year) { ElMessage.warning('请选择会计年度'); return; }
  try {
    await ElMessageBox.confirm(
    ...
    if (!preview.canExecute) {
      ...
      addToHistory('inventoryYearEnd', `${inventoryYearEndForm.year}年度`, 'failed', '预览检查未通过');
      return;
    }
    const response = await inventoryApi.executeYearEnd({ year: inventoryYearEndForm.year });
    if (response?.data) {
...
const freezeInventoryYearEnd = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      ...
      '确认冻结结存',
      { confirmButtonText: '确定冻结', cancelButtonText: '取消', type: 'warning' }
    );
    inventoryFreezeLoading.value = true;
    const response = await inventoryApi.freezeYearEnd({ year: inventoryYearEndForm.year });
    if (response?.data) {
...
.status-name { font-size: 14px; font-weight: 500; color: var(--color-text-regular); }
.status-card-body { display: flex; align-items: center; justify-content: space-between; }
.status-schedule { font-size: 12px; color: var(--color-text-secondary); }
...
.task-card-header { display: flex; align-items: flex-start; gap: 12px; margin-bottom: 16px; }
.task-icon { font-size: 28px; flex-shrink: 0; margin-top: 2px; }
.task-info h4 { margin: 0 0 4px 0; font-size: 15px; font-weight: 600; color: var(--color-text-primary); }
.task-info p { margin: 0; font-size: 12px; color: var(--color-text-secondary); }
.task-card-body { padding-top: 4px; }

/* 年度结存卡片 */
.year-end-card { min-height: 280px; }
.year-end-card .task-card-body { padding-top: 8px; }

.status-info-box { padding: 12px; background-color: var(--color-bg-section); border-radius: 6px; margin-bottom: 16px; }
.status-row { display: flex; align-items: center; gap: 8px; margin-bottom: 8px; }
.status-row:last-child { margin-bottom: 0; }
.status-row .status-label { font-size: 12px; color: var(--color-text-secondary); }
...
/* 生产成本区域 */
.production-cost-section { padding: 4px 0; }
...
.repair-form { display: flex; gap: 8px; align-items: flex-end; flex-wrap: wrap; }
...
:deep(.el-table th) { background-color: var(--color-bg-light) !important; font-weight: 600; }
</style>
