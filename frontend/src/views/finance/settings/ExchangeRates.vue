    ...
    >
      <el-form :model="form" label-width="90px">
        <el-form-item label="源币种" required>
          <el-select v-model="form.fromCurrency" filterable class="w-full">
            <el-option v-for="c in currencies" :key="c" :label="c" :value="c" />
          </el-select>
        </el-form-item>
        <el-form-item label="目标币种"><el-input v-model="form.toCurrency" placeholder="默认 CNY" /></el-form-item>
        <el-form-item label="汇率" required><el-input-number v-model="form.rate" :precision="6" :min="0" class="w-full" /></el-form-item>
        <el-form-item label="生效日期" required><el-date-picker v-model="form.effectiveDate" type="date" value-format="YYYY-MM-DD" class="w-full" /></el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="formVis = false">取消</el-button>
        <el-button v-permission="'finance:exchange-rates:update'" type="primary" @click="handleSave" :loading="saving">保存</el-button>
      </template>
        </AppDialog>
  </div>
</template>
...
const openForm = () => { form.value = { fromCurrency: 'USD', toCurrency: 'CNY', rate: 0, effectiveDate: '' }; formVis.value = true }

const handleSave = async () => {
  if (!form.value.rate || !form.value.effectiveDate) return ElMessage.warning('请填写完整')
  saving.value = true
