    ...
    <el-table v-loading="loading" :data="rows" border stripe>
      ...
      <el-table-column prop="financeStatus" label="财务状态" width="110">
        <template #default="{ row }">
          <el-tag :type="statusType(row.financeStatus)">{{
            statusLabel(row.financeStatus)
          }}</el-tag>
        </template>
      </el-table-column>
      ...
      <template v-if="detail">
        <el-descriptions :column="3" border>
          ...
          <el-descriptions-item label="财务状态">{{
            statusLabel(detail.financeStatus)
          }}</el-descriptions-item>
          <el-descriptions-item label="快照哈希" :span="3">{{
            detail.snapshotHash || '-'
          }}</el-descriptions-item>
        </el-descriptions>
...
const statusLabel = (value) =>
  ({ pending: '待审核', approved: '已通过', rejected: '已驳回', reversed: '已反审核' })[value] ||
  value ||
  '-';
const statusType = (value) =>
  ({ pending: 'warning', approved: 'success', rejected: 'danger', reversed: 'info' })[value] || '';

async function loadList() {
  loading.value = true;
  try {
    const response = await financeApi.inventoryPostings.list({
      ...filters,
...
async function approve(row) {
  await ElMessageBox.confirm(`确认审核过账 ${row.sourceNo}？`, '审核确认');
...
async function reject(row) {
  const { value } = await ElMessageBox.prompt('请输入驳回原因', '驳回过账', {
    inputValidator: (input) => Boolean(String(input || '').trim()) || '驳回原因不能为空',
  });
...
async function reverse(row) {
  const { value } = await ElMessageBox.prompt('请输入反审核原因', '反审核确认', {
    inputValidator: (input) => Boolean(String(input || '').trim()) || '反审核原因不能为空',
  });
  await financeApi.inventoryPostings.reverse(row.id, { remark: value });
  ElMessage.success('已提交反审核申请');
  await loadList();
}
